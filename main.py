# -*- coding: UTF-8 -*-
## learn Python2 in Y mins (簡體中文)  範例程式
...
# 從命令列獲得值也很方便
input_string_var = input("Enter some data: ") # 資料會被視為字串存進變數
#Error> input_var = input("Enter some data: ") # 輸入的資料會被當作Python程式碼執行
# 注意: 請謹慎使用input()函式
...
# 對沒有被賦值的變數取值會造成例外
# 請參考錯誤流程部分做例外處理
#Error> some_other_var  # 造成 NameError
...
# 存取超過範圍會產生IndexError
#Error> li[4]  # Raises an IndexError
...
#Error> li.remove(2)  # 2 不在串列中，造成 ValueError
...
# 取得特定值在串列中第一次出現的位置
li.index(2)  # => 1
#Error> li.index(7)  # 7 不在串列中，造成 ValueError
...
#Error> filled_dict["four"]   # KeyError
...
#Error> with open("myfile.txt") as f:
#Error>    for line in f:
#Error>        print line
...
# 移除屬性
del i.age
#Error> i.age  # => raises an AttributeError
...
# python 3.x 沒有 xrange, 改回用 range
xrange_ = range(1, 900000000)
